...
# 일단 문제 빈칸부터 제작
    for ans in range(trial): 
        ...
        guess = input(f"{question} (trial={trial}) 답을 입력하시오 => ")
        ...
        if "_" not in answer_list:
            print (f"축하합니다!!!!!!!!!! 정답 {quiz}를 맞추셧습니다!!")
            break # for반ㄴ복문 안에 잇어야 사용가능
        #중단하고 성공했다고 알리기
         

    # 기회가 끄탐
    if trial == 0:
        print(f"게임 오버! 아쉽지만 기회를 모두 소진했습니다. 정답은 '{quiz}' 였습니다.")
